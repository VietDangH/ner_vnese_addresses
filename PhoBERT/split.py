...
separate step (``vn_ner.augment``) so you can augment any file you like — not
...
Writes ``<out_dir>/{train,val,test}.conll`` (all clean). Then, optionally:
    ...
    python -m vn_ner.train --train_data data_split/train_aug.conll \
...
def parse_args():
    ...
    p.add_argument("--data", default="Cleaned_Data.conll")
...
def main():
    ...
    print(f"    python -m vn_ner.augment --data {paths['train']} "
          f"--out {a.out_dir}/train_aug.conll")
    print("[i] Train:")
    print(f"    python -m vn_ner.train --train_data {paths['train']} "
          f"--val_data {paths['val']}")
    print("[i] Evaluate:")
    print(f"    python -m vn_ner.evaluate --model_dir ner_model "
          f"--data {paths['test']}")
