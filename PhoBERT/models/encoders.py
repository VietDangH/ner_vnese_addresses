# -*- coding: utf-8 -*-
"""Optional *top encoders* — extra contextual layers stacked on the PhoBERT
word features before the head.

This is the axis along which you "easily add other layers like LSTM, CNN". Each
top encoder maps word features ``(B, W, in_dim)`` -> ``(B, W, out_dim)`` and
exposes its ``out_dim`` so the head can size its projection accordingly.

    top_encoder="none"    -> identity (PhoBERT features straight into the head)
    top_encoder="bilstm"  -> a BiLSTM over the word sequence
    top_encoder="cnn"     -> a stack of 1-D convolutions over the word sequence
...
class Identity(nn.Module):
    """Pass PhoBERT word features straight through to the head."""
...
class BiLSTM(nn.Module):
    """A (multi-layer) bidirectional LSTM over the word sequence."""
    ...
    def forward(self, x, mask):
        # pack so the LSTM ignores padded words, then pad back to W.
...
class CNN(nn.Module):
    """A stack of width-preserving 1-D convolutions over the word sequence."""
    ...
    def forward(self, x, mask):
        ...
        return h * mask.unsqueeze(-1).to(h.dtype)          # zero padded words
