# -*- coding: utf-8 -*-
"""The composable NER model: PhoBERT encoder -> optional top layer -> head.

    sub-word ids ─► PhoBERT ─► last_hidden_state (B, T, H)
                                 │  gather first sub-word per word
                                 ▼
                         word features (B, W, H)
...
class NERModel(nn.Module):
    ...
    def decode_with_confidence(self, input_ids, attention_mask, word_positions,
                               word_mask):
        """Decode and attach a per-word confidence ``P(predicted_tag | x)``.
