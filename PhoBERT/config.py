...
DEFAULT_DATA = "data_for_training.conll"
DEFAULT_OUTPUT_DIR = "ner_model"

# Architecture choices (see vn_ner.models). Pick the head to train "with or
# without CRF"; pick a top encoder to stack BiLSTM/CNN on the PhoBERT features.
...
class TrainConfig:
    ...
    are separate steps — see ``vn_ner.split`` and ``vn_ner.augment``). For quick
    experiments, leave ``train_data``/``val_data`` unset and pass a single
    ...
    freeze_encoder: bool = False              # train head/top only (probe PhoBERT)
