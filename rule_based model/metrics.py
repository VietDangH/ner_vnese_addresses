# -*- coding: utf-8 -*-
"""Detailed performance reporting for the rule-based baseline.

This is a **verbatim copy** of :mod:`vn_ner.metrics` (kept local so the baseline
has zero torch dependency). Because the code is identical, the report and JSON
this baseline emits have exactly the same structure as the PhoBERT / BamiBERT
``eval_report.txt`` / ``eval_metrics.json`` — the numbers slot straight into the
same comparison table.

``seqeval`` gives entity-level (span-level) precision / recall / F1 -- a span
counts as correct only if its boundaries *and* type both match. On top of that:
...
def compute_metrics(y_true: List[List[str]], y_pred: List[List[str]]) -> Dict:
    ...
    # token-level accuracy across all words
