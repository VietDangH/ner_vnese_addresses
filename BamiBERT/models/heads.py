# -*- coding: utf-8 -*-
"""Decoder *heads* — the part that turns per-token features into tag predictions.
...
needs to know which one is in use::
    ...
    decode(feats, mask)                    -> List[List[int]]   (trimmed to tokens)
    decode_with_confidence(feats, mask)    -> (preds, confidences)

`feats` is `(B, W, in_dim)`, `mask` is `(B, W)` bool (True = real token),
`labels` is `(B, W)` long. Outputs are Python lists with padding removed.
...
def _trim(tensor, mask):
    """(B, W) tensor -> list of per-sentence lists, padded tokens removed."""
...
class SoftmaxHead(nn.Module):
    ...
    Scores each token independently, so it is cheap at inference but can emit
