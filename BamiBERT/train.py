...
CRF", the top encoder optionally stacks a BiLSTM/CNN on the BamiBERT features:

    # BamiBERT + CRF (default)
    python -m bami_ner.train --train_data train.conll --val_data val.conll

    # BamiBERT only (no CRF)
    python -m bami_ner.train --train_data train.conll --val_data val.conll --head softmax

    # BamiBERT + BiLSTM + CRF
    python -m bami_ner.train --train_data train.conll --val_data val.conll \
        --top_encoder bilstm --lstm_hidden 256 --head crf

Input must be RAW (un-segmented) CoNLL — run ``desegment.py`` first if your data
is word-segmented. Data splitting and noise augmentation are separate steps — see
``bami_ner.split`` and ``bami_ner.augment``.
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Train a BamiBERT NER model (choose head + optional top layer)")
    # data
    ...
    p.add_argument("--top_encoder", default=C.DEFAULT_TOP_ENCODER,
                   ...
                   help="optional layer on top of BamiBERT before the head")
    p.add_argument("--freeze_encoder", action="store_true",
                   help="freeze BamiBERT; train only the top/head layers")
