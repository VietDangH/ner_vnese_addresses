...
``bami_ner.evaluate`` (run it on the held-out test file afterwards). Data
splitting and noise augmentation are also separate steps (``bami_ner.split`` /
``bami_ner.augment``); the trainer just consumes the TRAIN and VAL files it is
given.
"""
...
def build_optimizer(model, lr_bert, lr_head, weight_decay):
    """Discriminative LRs: BamiBERT encoder weights vs. the fresh top/head layers,
...
def train(cfg: TrainConfig):
    ...
    print(f"Tokenizer: byte-level (add_prefix_space)={collate.add_prefix_space}")
    ...
    print(f"Next: python -m bami_ner.evaluate --model_dir {cfg.output_dir} "
