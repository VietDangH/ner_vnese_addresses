...
model forward pass:
    ...
    raw text ─► whitespace/punct tokenization ─► model ─► tags

BamiBERT consumes **raw** text, so — unlike the PhoBERT pipeline — there is no
word-segmentation step: the prep here is only a cheap whitespace/punctuation
split (``rawtext.raw_tokenize``). It is still included in (and broken out of)
every number so the two packages are measured the exact same way. The test
CoNLL tokens are re-joined into a raw string first so the tokenizer has real
work to do, exactly as at serving time.

Two views:
  * batched throughput  — tokenize every sentence, then run the model in batches
    at ``--batch_size``; reported end-to-end and split into tokenization vs. model.
  * single-sentence     — tokenize + decode one sentence at a time (latency):
    mean / median / p95 ms, plus the mean tokenization share.
...
# --------------------------------------------------------------------------- #
# Package-specific raw-text prep — the ONLY part that differs from vn_ner.
# --------------------------------------------------------------------------- #
PREP_STAGE = "tokenization"          # what the raw-text prep step does


def get_backend() -> str:
    """BamiBERT needs no word segmenter; the prep is a plain whitespace split."""
    return "whitespace+punct"


def prepare(text: str) -> list:
    """Raw address string -> model-ready tokens (whitespace/punct split, no seg)."""
    return raw_tokenize(text)


def to_raw_text(tokens: list) -> str:
    """Rebuild a raw address string from CoNLL tokens (already raw syllables)."""
    return " ".join(tokens)
...
def benchmark(model, collate, device, raw_texts, warmup=2, use_amp=False,
    ...
    # Warm up the full path (warms tokenizer caches + CUDA kernels, batched and
    # single) so the timed sections don't pay one-off startup costs.
...
def parse_args():
    ...
    p.add_argument("--data", default="data_split/test.conll",
                   help="CoNLL file to time on; its tokens are re-joined into raw "
                        "text so tokenization is part of the timing")
