...
Operates on this package's sentence representation -- ``(tokens, labels)``
parallel lists -- so the trainer can augment the TRAIN split in-process (split
first, noise only train -> no leakage into val/test).

Noise types (each with its own probability):
  * abbreviate keyword  : ngõ->ng. , quận->q , xã->x ...
...
Note on raw (de-segmented) input: BamiBERT input is un-segmented, so multi-
syllable heads appear as separate tokens (e.g. ``thành`` ``phố``). The
single-syllable keyword ops below (``quận``, ``xã``, ``đường``, ...) still fire
as usual; the compound entries (``thành_phố``, ``thị_trấn``, ``thị_xã``) simply
never match on raw tokens, which is harmless.
